    ...
    namespace HM
    {
       ...
       {
               //LOG_DEBUG("TCPConnection::GetSocket()");
          if (_useSSL)
    //                if (_handshakeDone == false){
    if ((_stateSTARTTLS == 1) || (_stateSTARTTLS == 2))
    {
     
                              //LOG_DEBUG("TCPCconnection::GetSocket(): next_layer()");
                              return _sslSocket.next_layer();
                      } else {
                              //LOG_DEBUG("TCPCconnection::GetSocket(): lowest_layer()");
    ...
    if (_stateSTARTTLS > 0)
    {
             ...
             //LOG_DEBUG("TCPConnection::~TCPConnection 4USESSL & TLS1");
    ...
    {
             //LOG_DEBUG("TCPConnection::~TCPConnection 5USESSL & TLS!1");
             ...
             {
             //LOG_DEBUG("TCPConnection::~TCPConnection 6!USESSL & !TLS");
                // Send welcome message to client.
                _protocolParser->OnConnected();
             }
          }
    ...
    if ((_stateSTARTTLS == 1))
    {
          //LOG_DEBUG("TCPConnection::~TCPConnection _stateSTARTTLS 1");
    ...
    else
    {
          ...
          //LOG_DEBUG("TCPConnection::~TCPConnection _stateSTARTTLS 0/2/3");
       ...
       TCPConnection::HandleHandshake(const boost::system::error_code& error)
       {
          try
          {
             if (!error)
             {
               ...
               //LOG_DEBUG("TCPConnection::HandleHandshake Completed");
    ...
    _stateSTARTTLS=3;                        
                ...
                // Send welcome message to client.
    //if (_stateSTARTTLS==0)  _protocolParser->OnConnected(); //temp hack to stop banner after starttls handshake
    if (_StartTLSEnabled){
    ...
    //         LOG_DEBUG("TCPConnection::_ProcessOperationQueue BEFORE");
                      if ((_stateSTARTTLS == 2) && (_handshakeDone == false)) return;
    //         LOG_DEBUG("TCPConnection::_ProcessOperationQueue AFTER");
          ...
          try
          {
    //         LOG_DEBUG("TCPConnection::_ProcessOperationQueue STAGE1");
             stage = 1;
             // Pick out the next item to process...
             shared_ptr<IOOperation> operation = _operationQueue.Front();
     
             stage = 2;
    //         LOG_DEBUG("TCPConnection::_ProcessOperationQueue STAGE2");
             if (!operation)
             {
    //         LOG_DEBUG("TCPConnection::_ProcessOperationQueue STAGE20");
                // We're no longer sending...
                stage = 20;
                return;
             }
             stage = 3;
    //         LOG_DEBUG("TCPConnection::_ProcessOperationQueue STAGE3");
     
            switch (operation->GetType())
             {
             case IOOperation::BCTSend:
                {
    //         LOG_DEBUG("TCPConnection::_ProcessOperationQueue STAGE4");
                   stage = 4;
                   shared_ptr<ByteBuffer> pBuf = operation->GetBuffer();
                   Write(pBuf);
    //         LOG_DEBUG("TCPConnection::_ProcessOperationQueue STAGE5");
             ...
             case IOOperation::BCTReceive:
                {
    //         LOG_DEBUG("TCPConnection::_ProcessOperationQueue STAGE6");
                   stage = 6;
                   Read(operation->GetString());              
    //         LOG_DEBUG("TCPConnection::_ProcessOperationQueue STAGE7");
                   stage = 7;
                   break;
                }
             case IOOperation::BCTShutdownSend:
                {
    //         LOG_DEBUG("TCPConnection::_ProcessOperationQueue STAGE8");
                   stage = 8;
                   Shutdown(boost::asio::ip::tcp::socket::shutdown_send, true);
                   stage = 9;
    //         LOG_DEBUG("TCPConnection::_ProcessOperationQueue STAGE9");
                   _ProcessOperationQueue();
                   stage = 10;
    //         LOG_DEBUG("TCPConnection::_ProcessOperationQueue STAGE10");
     
                   break;
                }
             case IOOperation::BCTDisconnect:
                {
    //         LOG_DEBUG("TCPConnection::_ProcessOperationQueue STAGE11");
                   stage = 11;
                   Disconnect();
                   stage = 12;
    //         LOG_DEBUG("TCPConnection::_ProcessOperationQueue STAGE12");
     
                   break;
                }
    //         LOG_DEBUG("TCPConnection::_ProcessOperationQueue STAGE13");
     
             }
    //         LOG_DEBUG("TCPConnection::_ProcessOperationQueue STAGE14");
    ...
    if ((_stateSTARTTLS == 1) || (_stateSTARTTLS == 2))
    {
                                            ...
                                            //LOG_DEBUG("TCPConnection::Write next_layer()");
    ...
    {
                                            //LOG_DEBUG("TCPConnection::Write ssl");
    ...
    {
                                            //LOG_DEBUG("TCPConnection::Write non ssl");
    ...
    if ((_stateSTARTTLS == 1)||(_stateSTARTTLS == 2))
    {
                                            ...
                                            //LOG_DEBUG("TCPConnection::Read next_layer()");
    ...
    else
    {
                                            ...
                                            //LOG_DEBUG("TCPConnection::Read ssl");
                ...
                else
                {
                                            ...
                                            //LOG_DEBUG("TCPConnection::Read non ssl");
    ...
    //LOG_DEBUG("TCPConnection::STARTTLS_Handshake handshake1");
    ...
    //      TCPConnection::stateSTARTTLS(2); // Change state to handshake complete
     
    //LOG_DEBUG("TCPConnection::STARTTLS_Handshake handshake3");
